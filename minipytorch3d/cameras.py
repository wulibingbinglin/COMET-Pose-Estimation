...
from typing import Any, Dict, List, Optional, Sequence, Tuple, Union # 用于类型注解，帮助定义代码的类型

import numpy as np # 用于数组操作
import torch # 深度学习和张量操作
import torch.nn.functional as F # 提供了 PyTorch 中的函数式操作
...
_R = torch.eye(3)[None]  # (1, 3, 3) #在张量索引时使用 [None]（也可以写作 unsqueeze(0)）会在第0维新增一个维度。新增的维度大小为 1。当你只处理一个摄像头的数据时，批量大小就是 1，因此旋转矩阵的形状是 (1, 3, 3)。
_T = torch.zeros(1, 3)  # (1, 3)

_BatchFloatType = Union[float, Sequence[float], torch.Tensor]
"""确定batch中的类型"""

#Python 的 Union 让你能够在类型上进行一些模糊的指定，表示接受多种不同的输入类型。
 # 定义：这个定义表示 _BatchFloatType 可以是以下几种类型之一：
#
# float：一个单一的浮点数。
# Sequence[float]：一个浮点数的序列，Sequence 是 Python 的一个抽象基类，表示一个有序的容器，可以是列表（list）、元组（tuple）等。
# torch.Tensor：PyTorch 中的张量（Tensor）。张量是一个多维数组，可以包含标量、向量、矩阵或者更高维度的数据。
# 用途：这是为了处理批量数据时的灵活性。例如，在神经网络训练过程中，有时候你需要一次处理多个数据样本（例如多个浮点数）。_BatchFloatType 允许：
#
# 一个浮点数（表示单个样本的浮动数据）。
# 一个浮点数序列（例如一个包含多个样本的列表）。
# 一个 PyTorch 张量（通常用于批量处理大量数据，因为 PyTorch 张量可以高效地处理大规模的数据并支持 GPU 加速）。
# 这使得函数或变量可以接受单个数据或多个数据样本，并且对数据进行统一处理，不需要写不同的代码来处理不同的数据结构。
...
"""确定焦距的类型"""
# float：单一的浮动数值，表示一个焦距值。
# Sequence[Tuple[float]]：一个包含浮动数值元组的序列。每个元组可以是一个浮动数值，表示多个焦距的可能组合（例如多个镜头的焦距）。
# Sequence[Tuple[float, float]]：一个包含二元组（每个元组包含两个浮动数值）的序列。这可能用于表示某些复杂的焦距参数，例如不同镜头的水平和垂直焦距。
# torch.Tensor：PyTorch 张量，用于表示一组焦距数据的批量（可能是多个焦距参数）。
# 为什么有时使用 Tuple 而不是直接使用 float？
# 假设你有如下的需求：
#
# 每个焦距值本身是单独的浮动数值，你可以直接使用 Sequence[float] 来表示。
# 如果你正在设计一个系统，可能会在未来希望每个元素有更多的数据（例如，每个焦距值不仅是一个数值，未来可能会有更多参数与焦距一起存储），使用 Tuple[float] 就预留了结构扩展的可能性。"""


class CamerasBase(TensorProperties):
    """ 四种主要的坐标系
        世界坐标系 (World): 对象所在的世界空间。
        摄像机坐标系 (View): 摄像机所在的空间，Z 轴垂直于图像平面。
         NDC 坐标系: 标准化坐标系，所有坐标被映射到 [-1, 1] 范围。
        像素坐标系 (Screen): 对应图像像素的实际坐标系。
    get_camera_center 返回相机的光学中心在世界坐标系中的位置。

    get_world_to_view_transform 返回从世界坐标到相机视图坐标的 3D 变换矩阵，包括旋转 (R) 和平移 (T)。

    get_full_projection_transform   返回完整的变换矩阵：组合了世界到视图变换 (R, T) 和投影变换 (P)。
    transform_points  将一组输入点从世界坐标投影到相机定义的空间（NDC 或屏幕空间）。
    get_ndc_camera_transform  定义从屏幕/NDC 坐标到 PyTorch3D NDC 坐标的变换。
    transform_points_ndc  将一组点从世界坐标投影到 PyTorch3D 的 NDC 坐标空间。
    transform_points_screen 将一组点从世界坐标投影到屏幕坐标空间。
    背景知识：
    每个方法对应一个特定的转换步骤，允许灵活地在不同坐标系之间进行切换和操作。
    模块 3：扩展性
    get_projection_transform 每个具体相机需要实现此方法，返回从相机视图坐标到相机坐标（NDC 或屏幕）的映射。
    unproject_points每个具体相机也需要实现此方法，将点从相机坐标（NDC 或屏幕）反投影回相机视图或世界坐标"""
    _FIELDS: Tuple[str, ...] = ()
    # 用于定义类中的字段。这些字段会在创建新的相机实例时被设置。

    _SHARED_FIELDS: Tuple[str, ...] = ()
    # 表示整个批量中共享的属性（即固定不变的属性）。
    # 在批量处理多个相机对象时，这些字段必须一致。
    """_FIELDS 和 _SHARED_FIELDS 是类的属性。
        它们的类型是一个元组，元组中的元素类型为字符串（str）。
        它们的默认值是空元组（()）
        类型注解是可选的："""
    ...
    def get_projection_transform(self, **kwargs):
        """
        **kwargs 是一种 Python 参数解包机制，
        允许在调用此方法时传入任意数量的关键字参数（即形如 key=value 的参数）。
        kwargs 中的参数可以覆盖在类初始化方法（__init__）中设置的默认值。
        这表明这个方法具有高度的灵活性，允许用户通过额外参数调整投影矩阵计算过程。
        ...
        """在面向对象编程中，如果某个类或接口定义了方法，但不负责具体实现，而是留给子类实现，
          可以用 raise NotImplementedError() 表示。"""
    ...
    def get_camera_center(self, **kwargs) -> torch.Tensor:
        ...
        Setting R or T here wil

        l update the values set in init as these
        ...
        P = w2v_trans.inverse().get_matrix()# 就当作这个点在摄像机坐标系下的坐标为000，然后其次坐标就是0001，就求逆嘛，这样就可以理解。
        C = P[:, 3, :3]
        """现在看一下 P[:, 3, :3] 这个操作，它表示：
        P[:, 3] 得到的是一个 4x1 的列向量，包含了矩阵 P 的第四列。 
        :3 表示选取 前三个元素，即选取向量中的前三个元素（这里是平移向量中的前三个元素）。"""
        return C
    ...
    def get_world_to_view_transform(self, **kwargs) -> Transform3d:
        ...
        # 这个 kwargs.get("R", self.R) 的意思是，如果外部调用该方法时传入了 R（旋转矩阵）参数，
        # 就使用传入的 R，否则就使用类实例 self 中已有的 R。
        self.R = R
        self.T = T
        world_to_view_transform = get_world_to_view_transform(R=R, T=T) #8个 Transform3d 对象，表示从 世界坐标系（World Space） 到 相机坐标系（View Space） 的变换
...
class PerspectiveCameras(CamerasBase):
    ...
    def __init__(
        ...
        self.focal_length = self.focal_length.expand(-1, 2)  # (N, 2) 通常是为了在后续的计算中分别对应图像的水平和垂直方向的焦距（假设它们相同）
...
def get_world_to_view_transform(
    ...
    a translation.该函数根据输入的 旋转矩阵 R 和 平移向量T，构造一个 Transform3d 对象，表示从 世界坐标系（World Space） 到 相机坐标系（View Space） 的变换。
    ...
    它使用了 PyTorch3D 中的 Rotate 和 Translate 类来构建相应的变换对象，并最终组合成一个 Transform3d 对象，这个对象可以用于执行实际的坐标变换
    """
