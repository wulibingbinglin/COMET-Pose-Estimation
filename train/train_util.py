...
# from co3d_v2 import Co3dDataset, TRAINING_CATEGORIES, Co3dDatasetFix
from torch.utils.data import BatchSampler
# from griddle.utils import is_aws_cluster

from datasets.megadepth import MegaDepthDataset
# from megadepthV2 import MegaDepthDatasetV2, MegaDepthDatasetV2Fix
...
def build_optimizer(cfg, model, dataloader):
    if cfg.train.warmup_sche: # 函数首先检查 cfg.train.warmup_sche 是否为 True，即是否启用 Warmup 学习率调度。
        if cfg.var_lr: # 如果启用 Warmup 学习率调度，并且 cfg.var_lr 为 True，则使用 torch.optim.AdamW 创建一个 AdamW 优化器，并对模型的不同部分使用不同的学习率：
            optimizer = torch.optim.AdamW(
                [
                    {"params": model.image_feature_extractor.parameters(), "lr": cfg.train.lr / 10}, # image_feature_extractor 部分使用 cfg.train.lr / 10 的学习率。
                    {
                        "params": [p for name, p in model.named_parameters() if "image_feature_extractor" not in name],
                        "lr": cfg.train.lr, # 其他部分使用 cfg.train.lr 的学习率。
                    },
                ]
            )
        else:
            optimizer = torch.optim.AdamW(params=model.parameters(), lr=cfg.train.lr) # 如果没有启用 var_lr，则所有模型参数使用相同的学习率 cfg.train.lr。
        ...
        # 如果没有启用 Warmup 调度，创建一个普通的 AdamW 优化器，并使用 torch.optim.lr_scheduler.LinearLR 来调度学习率。
        # 这个调度器会线性地从初始学习率 cfg.train.lr 降低到零。
        ...
        # 代码中还包括了一些其他可能的学习率调度器，如：
        # StepLR：每经过一定的步数（step_size）后，学习率按 gamma 参数衰减。
        # OneCycleLR：一种动态调整学习率的策略，可以在训练过程中从较低的学习率逐步提升到最大值，然后再下降。
