...
def generate_samples(N, target_num, sample_num, expand_ratio=2):
    """
    This function generates random samples of indices without duplicates. 是生成不重复的随机样本索引

    Parameters:
    N (int): The upper limit for generating random integers.  生成随机整数的上限
    max_num_trials (int): The maximum number of trials for generating samples. 最终希望返回的样本数。
    sample_num (int): The number of samples to generate. 用于扩展生成样本的比例，默认为 2。也就是说，
    会先生成 target_num * expand_ratio 个样本，接着筛选出没有重复的样本，再缩小到最终的 target_num 个样本。
    Returns:
    ...
    return sample_idx_safe # 最终返回一个形状为 (target_num, sample_num) 的数组 4096 7
...
def get_default_intri(width, height, device, dtype, ratio=1.0):
    # assume same focal length for hw 于生成默认的相机内参矩阵（Intrinsic Matrix）及相关参数
    max_size = max(width, height)
    focal_length = max_size * ratio # # 计算焦距：max_size * ratio 假设：水平和垂直方向的焦距相同

    principal_point = [width / 2, height / 2] # 表示光心在图像中的坐标 [c_x, c_y]

    K = torch.tensor(
        [
            [focal_length, 0, width / 2],  # 行1: [f_x, 0, c_x]
            [0, focal_length, height / 2],  # 行2: [0, f_y, c_y]
            [0, 0, 1],           # 行3: [0, 0, 1]
        ],
        device=device,
        dtype=dtype,
