...
def rotation_angle(rot_gt, rot_pred, batch_size=None, eps=1e-15):
    ...
    ###################new#######################
    # 确保四元数归一化
    pred_q = F.normalize(q_pred, dim=-1)
    gt_q = F.normalize(q_gt, dim=-1)

    # 计算四元数内积
    dot_product = (pred_q * gt_q).sum(dim=-1)

    # 考虑双覆盖性质：取绝对值
    dot_product = torch.abs(dot_product)

    # 计算旋转角度损失
    # 使用 1 - dot_product^2 作为损失，避免数值不稳定
    angle_loss = 1 - dot_product ** 2
    err_q = torch.clamp(angle_loss, min=eps)
