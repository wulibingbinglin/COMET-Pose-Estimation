...
这个12 就是将原先的三头输出 改为了一个mlp输出7个参数  对应的就是 消融实验  主要链接的是 参数和这两新文件的添加 camera_to_pose_encoding3, pose_encoding_to_camera3
"""
...
from .utils import (
    ...
    camera_to_pose_encoding, get_1d_sincos_pos_embed, camera_to_pose_encoding3, pose_encoding_to_camera3,
)
...
class CameraPredictor(nn.Module):
    ...
    def forward(
        ...
        # multi-head attention 提取更加抽象的特征表示
        rgb_feat = self.trunk(rgb_feat)

        # 预测相机位姿预测，直接单头输出
        pred_pose_enc = self.pose_branch(rgb_feat)

        # 规则：四元数部分归一化
        pred_pose_enc = pred_pose_enc.clone()
        pred_pose_enc[..., 3:7] = F.normalize(
            pred_pose_enc[..., 3:7].clone(), 
            p=2, dim=-1, eps=1e-8
        )
        ...
        if gt_cameras is not None:
            # 将真值转换为编码格式
            gt_pose_enc = camera_to_pose_encoding3(gt_cameras, pose_encoding_type=self.pose_encoding_type)
            
            # 权重分配规则：旋转通常比平移更敏感，赋予更高权重
            weight_trans = self.cfg.get("weight_trans", 1.0)
            weight_rot = self.cfg.get("weight_rot", 2.0)
            
            # 提取预测与真值 (去除第一帧，因为第一帧通常作为参考帧)
            pred_trans = pred_pose_enc[:, 1:, :3].clone().reshape(-1, 3)
            gt_trans = gt_pose_enc[1:, :3].clone()
            pred_rot = pred_pose_enc[:, 1:, 3:7].clone().reshape(-1, 4)
            gt_rot = gt_pose_enc[1:, 3:7].clone()

            # Loss 计算规则：平移和旋转均统一乘以 10^2 以放大梯度
            trans_loss = F.mse_loss(pred_trans, gt_trans) * (10 ** 2)
            rot_loss = F.mse_loss(pred_rot, gt_rot) * (10 ** 2)
        ...
        # 强制第一帧归零规则（相对姿态）
        pred_pose_enc[:, 0, :3] = 0
        pred_pose_enc[:, 0, 3:7] = torch.tensor([1, 0, 0, 0], device=pred_pose_enc.device)

        # 将编码转回相机对象
        pred_cameras = pose_encoding_to_camera3(
            pred_pose_enc,
        ...
        return {
            "pred_pose_enc": pred_pose_enc.reshape(-1, 7),
            "gt_pose_enc": gt_pose_enc if gt_cameras is not None else None,
