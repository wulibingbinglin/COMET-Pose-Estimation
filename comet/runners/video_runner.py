...
class VideoRunner(VGGSfMRunner):
    ...
    def run(
        ...
        with torch.no_grad():
            ...
            # Add batch dimension if necessary  如果输入 images 的形状为 (T, C, H, W)（缺少 Batch 维度），则调用 add_batch_dimension 添加一个维度
            if len(images.shape) == 4:
                images = add_batch_dimension(images)
                masks = add_batch_dimension(masks)
                crop_params = add_batch_dimension(crop_params)

            # Calculate bounding boxes if crop parameters are provided calculate_bounding_boxes 计算用于 3D 估计的裁剪边界框（bounding boxes）
            ...
            B, T, C, H, W = images.shape # （1,8 ，3，1024，1024）
            self.B = B
            self.img_dim = C
            self.H = H
            self.W = W
            ...
            start_idx = 0
            end_idx = start_idx + init_window_size # 处理 init_window_size 数量的帧 32

            # Temporally double the max_query_pts to ensure
            # self.cfg.max_query_pts 存储了用于跟踪的最大特征点数量。
            # 这里临时将 max_query_pts 翻倍，以确保初始窗口内有足够的点进行跟踪
            ...
            # process_initial_window 是 VideoRunner 类中的一个方法，负责初始化 3D 估计。
            ...
            self.cfg.max_query_pts = self.cfg.max_query_pts // 2
            # 由于之前将 max_query_pts 翻倍，这里将其恢复到原值，以便后续处理。
            ...
            # 将初步的稀疏重建预测（init_pred）转换为点云框架的字典表示。
            ...
            while end_idx < T:
                ...
                # 在窗口的结束索引 end_idx 小于总帧数 T 时，继续执行滑动窗口的操作。
                # 如果剩余的帧数小于或等于窗口大小的 1.25 倍，则窗口的大小会相应地调整为剩余的帧数，否则窗口大小保持为 window_size。

                if not move_success:
                    # 如果窗口移动失败，程序会打印一条错误信息，并将 max_query_pts 翻倍，然后再次尝试移动窗口。
                    # 如果再次成功，则恢复 max_query_pts 的值。max_query_pts 可能是一个控制查询点数量的配置参数。
                    print(
                ...
                if window_counter % joint_BA_interval == 0:
                    # 每当 window_counter 达到 joint_BA_interval 的倍数时，就会执行一次联合束束调整（Joint Bundle Adjustment，BA）。
    ...
    def process_initial_window(
        self,
        start_idx,
        end_idx, # 窗口的起始和结束索引，用于确定从图像序列中提取哪一部分数据。
        images,
        masks,
        crop_params,
        image_paths,
        ...
        ) # extract_window 是一个辅助函数，它根据提供的 start_idx 和 end_idx
        # 提取图像数据（images）、掩码（masks）和裁剪参数（crop_params）的子集。它将返回从 start_idx 到 end_idx 之间的数据，这些数据将用于后续的处理。
        ...
        init_pred = self.sparse_reconstruct(
            ...
            back_to_original_resolution=False, # 这个不同 这是flase
        )
        return init_pred
    ...
    def move_window(
        ...
        # 计算 last_window_size，即上一个窗口的大小（end_idx - start_idx）。接下来，通过 assert 语句确保窗口的大小是正数。
        ...
        # extract_window 函数用于从图像序列中提取一个窗口的图像。
        # 这里提取的是包括了前一个窗口（last_start_idx）和当前窗口（start_idx 到 end_idx）的图像。提取出来的图像保存在 two_window_images 中。

        # Predict extri for the combination of (last_window, current_window)
        pred_cameras = average_camera_prediction( # 然后使用 average_camera_prediction 函数来预测与这两个窗口相关的相机参数
        ...
        ( # self.prepare_window_data 函数用于准备当前窗口的各种数据
        ...
        # 这段代码的核心任务是检查窗口中每一帧的有效性，如果发现某帧有效的内点少于阈值（50），则会尝试缩小窗口。如果窗口内没有足够有效的帧，代码会回退到前一个窗口，并停止当前处理。
        ...
        align_extri_window_plus_one = self.align_next_window(
            ...
            window_vis_inlier_for_exist_points,
            last_end_visible_points_3D,
        ) # 将当前窗口的外参对齐到下一个窗口。这包括使用已有的点和轨迹来调整外参，使得它们更适合当前窗口中的数据

        ( # 筛选出在当前窗口中有效的3D点，并根据这些点计算它们的有效掩码。掩码是用于标记哪些点是有效的，哪些点在计算中需要排除。
            exist_valid_points,
            exist_valid_tracks,
        ...
        ( # 对新图像中的点进行三角化，生成新的3D点，并计算这些点的轨迹、掩码和可见性。
